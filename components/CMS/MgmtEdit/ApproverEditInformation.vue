  ...
  <div class=" max-w-3xl">
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="Name">
        <FormControl
          :model-value="Approver.profile.name"
      ...
      <FormField label="E-mail">
        <FormControl
          :model-value="Approver.profile.email"
      ...
      <FormField label="Mobile">
        <FormControl
          :model-value="Approver.profile.mobile"
          placeholder="Enter Approver Mobile"
        />
      </FormField>
      <FormField label="Role">
        <FormControl
          :model-value="Approver.profile.role"
          :options="options.profileRoles"
        />
      </FormField>
      <FormField label="State">
        <FormControl
          :model-value="Approver.profile.state"
          :options="options.state"
        />
      </FormField>
      <FormField label="Language">
        <FormControl
          :model-value="Approver.profile.language"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="User Segment">
        <FormControl
          :model-value="Approver.AdditionalDetails.UserSegment"
          :options="options.UserSegment"
        />
      </FormField>
      <FormField label="Lead Status">
        <FormControl
          :model-value="Approver.AdditionalDetails.LeadStatus"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Approver.ReferralDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>

    </div>

    <h2 class="text-2xl mt-8 font-semibold">UTM Details</h2>
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Approver.UTMDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>
...
const items = ref(props?.Approver?.LoginDetails?.['last logins'])
