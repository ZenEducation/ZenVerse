  ...
  <div class=" max-w-3xl">
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="Name">
        <FormControl
          :model-value="Creator.profile.name"
      ...
      <FormField label="E-mail">
        <FormControl
          :model-value="Creator.profile.email"
      ...
      <FormField label="Mobile">
        <FormControl
          :model-value="Creator.profile.mobile"
          placeholder="Enter Creator Mobile"
        />
      </FormField>
      <FormField label="Role">
        <FormControl
          :model-value="Creator.profile.role"
          :options="options.profileRoles"
        />
      </FormField>
      <FormField label="State">
        <FormControl
          :model-value="Creator.profile.state"
          :options="options.state"
        />
      </FormField>
      <FormField label="Language">
        <FormControl
          :model-value="Creator.profile.language"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="User Segment">
        <FormControl
          :model-value="Creator.AdditionalDetails.UserSegment"
          :options="options.UserSegment"
        />
      </FormField>
      <FormField label="Lead Status">
        <FormControl
          :model-value="Creator.AdditionalDetails.LeadStatus"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Creator.ReferralDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>

    </div>

    <h2 class="text-2xl mt-8 font-semibold">UTM Details</h2>
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Creator.UTMDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>
...
const items = ref(props?.Creator?.LoginDetails?.['last logins'])
