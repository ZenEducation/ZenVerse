  ...
  <div class=" max-w-3xl">
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="Name">
        <FormControl
          :model-value="Editor.profile.name"
      ...
      <FormField label="E-mail">
        <FormControl
          :model-value="Editor.profile.email"
      ...
      <FormField label="Mobile">
        <FormControl
          :model-value="Editor.profile.mobile"
          placeholder="Enter Editor Mobile"
        />
      </FormField>
      <FormField label="Role">
        <FormControl
          :model-value="Editor.profile.role"
          :options="options.profileRoles"
        />
      </FormField>
      <FormField label="State">
        <FormControl
          :model-value="Editor.profile.state"
          :options="options.state"
        />
      </FormField>
      <FormField label="Language">
        <FormControl
          :model-value="Editor.profile.language"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <FormField label="User Segment">
        <FormControl
          :model-value="Editor.AdditionalDetails.UserSegment"
          :options="options.UserSegment"
        />
      </FormField>
      <FormField label="Lead Status">
        <FormControl
          :model-value="Editor.AdditionalDetails.LeadStatus"
    ...
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Editor.ReferralDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>

    </div>

    <h2 class="text-2xl mt-8 font-semibold">UTM Details</h2>
    <div class="p-6 bg-slate-100 dark:bg-transparent mt-5">
      <p v-for="(value , key) in Editor.UTMDetails" class="py-2">{{key}}: <span class="font-bold">{{value}}</span></p>
...
const items = ref(props?.Editor?.LoginDetails?.['last logins'])
