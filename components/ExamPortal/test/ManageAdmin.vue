...
import {
  createAdmin,
  createTestSeriesAdmin,
  deleteTestSeriesAdmin,
} from "@/src/graphql/mutations";
import { listTestSeriesAdmins, listAdmins } from "@/src/graphql/queries";
  ...
  try {
    ...
    if (resp && resp.data) {
      let relation = await API.graphql({
        query: createTestSeriesAdmin,
        variables: {
          input: {
            testSeriesId: props.postId,
            adminId: resp.data.createAdmin.id,
          },
        },
      });
      items.value.push(resp.data.createAdmin);
    }
  ...
  try {
    let relation = await API.graphql({
      ...
      variables: {
        input: {
          testSeriesId: props.postId,
          adminId: existingListSelectedId.value,
        },
      },
    });
...
onMounted(async () => {
  try {
    let MTAdmins = [];
    let testSeriesAdmins = await API.graphql({
      query: listTestSeriesAdmins,
      variables: {
        filter: { testSeriesId: { eq: props.postId } , _deleted:{ne:true} },
      },
    });
    testSeriesAdmins = testSeriesAdmins.data?.listTestSeriesAdmins?.items;
    console.log(testSeriesAdmins);
    MTAdmins = testSeriesAdmins
      .filter((item) => {
        return (
          !item._deleted && !item.admin?._deleted && !item?.testSeries?._deleted
        );
      })
      .map((item) => {
    ...
    testSeriesAdmins = testSeriesAdmins.map((item) => item.adminId);
    console.log(testSeriesAdmins);

    let allAdmins = await API.graphql({
      query: listAdmins,
    ...
    allAdmins.data?.listAdmins?.items.forEach((item) => {
      console.log(item.id);
      console.log(testSeriesAdmins.findIndex((t) => t == item.id));
      if (testSeriesAdmins.findIndex((t) => t == item.id) > -1) {
      } else {
        nonMTAdmins.push(item);
      }
    });

    console.log(MTAdmins, nonMTAdmins);
    items.value = MTAdmins;
    addExistingsList.value = nonMTAdmins;
  } catch (error) {
...
const userRoleOptions = ["All", "Admin", "Super Admin"];
const userRoleSelected = ref("All");
...
const filteredItems = computed(() => {
  ...
  if (userRoleSelected.value != "All") {
    filtered = filtered.filter((item) => {
      return item.role == userRoleSelected.value;
    });
  }
...
<template>
  <CardBoxModal v-model="isModalActive" :showFooter="false" title="">
    ...
    <p v-if="error" class="text-red-600">
      {{ `error creating new Admin : ${error}` }}
    </p>
  ...
  <CardBoxModal v-model="isModalExistingActive" :showFooter="false" title="">
    ...
    <CardBox is-form @submit.prevent="addExistingAdmin">
      ...
      <div class="lg:flex justify-between">
        <div class="flex items-start gap-y-4 flex-wrap">
          ...
          <div class="relative mr-4">
            ...
            >
              <PremFormField class="xl:mb-0 min-w-[50%] xl:min-w-[20%]">
                <label v-for="item in userRoleOptions">
                  <input
                    type="radio"
                    :value="item"
                    class="my-2"
