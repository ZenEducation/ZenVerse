...
import { createAffiliate, createTestSeriesAffiliate , deleteTestSeriesAffiliate } from "@/src/graphql/mutations";
import { listTestSeriesAffiliates, listAffiliates } from "@/src/graphql/queries";
  ...
  try {
    ...
    if (resp && resp.data) {
      let relation = await API.graphql({
        query: createTestSeriesAffiliate,
        variables: {
          input: {
...
onMounted(async () => {
  try {
    let MTAffiliates = [];
    let testSeriesAffiliates = await API.graphql({
      query: listTestSeriesAffiliates,
      variables: {
        filter: { testSeriesId: { eq: props.postId } , _deleted:{ne:true} },
      },
    });
    testSeriesAffiliates = testSeriesAffiliates.data?.listTestSeriesAffiliates?.items;
    console.log(testSeriesAffiliates);
    MTAffiliates = testSeriesAffiliates
      .filter((item) => {
        return (
          !item._deleted && !item.affiliate._deleted && !item.testSeries._deleted
        );
      })
      .map((item) => {
        return { ...item.affiliate , relationId: item.id, version: item._version };
      });
    testSeriesAffiliates = testSeriesAffiliates.map((item) => item.affiliateId);
    console.log(testSeriesAffiliates);
    ...
    allAffiliates.data?.listAffiliates?.items.forEach((item) => {
      console.log(item.id);
      console.log(testSeriesAffiliates.findIndex((t) => t == item.id));
      if (testSeriesAffiliates.findIndex((t) => t == item.id) > -1) {
      } else {
...
const deleteItem = async (popup, id, version) => {
  ...
  if (index !== -1) {
    try {
      ...
      await API.graphql({
        query: deleteTestSeriesAffiliate,
        variables: {
          input: {
