...
import { createInstructor, createTestSeriesInstructor , deleteTestSeriesInstructor } from "@/src/graphql/mutations";
import { listTestSeriesInstructors, listInstructors } from "@/src/graphql/queries";
  ...
  try {
    ...
    if (resp && resp.data) {
      let relation = await API.graphql({
        query: createTestSeriesInstructor,
        variables: {
          input: {
            testSeriesId: props.postId,
            instructorId: resp.data.createInstructor.id,
          },
        },
      });
      items.value.push(resp.data.createInstructor);
    }
...
const addExistingInstructor = async () => {
  try {
    let relation = await API.graphql({
      query: createTestSeriesInstructor,
      variables: {
        input: {
          testSeriesId: props.postId,
          instructorId: existingListSelectedId.value,
        },
      },
    });
...
onMounted(async () => {
  try {
    let MTInstructors = [];
    let testSeriesInstructors = await API.graphql({
      query: listTestSeriesInstructors,
      variables: {
        filter: { testSeriesId: { eq: props.postId } , _deleted:{ne:true} },
      },
    });
    testSeriesInstructors = testSeriesInstructors.data?.listTestSeriesInstructors?.items;
    console.log(testSeriesInstructors);
    MTInstructors = testSeriesInstructors
      .filter((item) => {
        return (
          !item._deleted && !item.instructor._deleted && !item.testSeries._deleted
        );
      })
      .map((item) => {
        return { ...item.instructor, relationId: item.id, version: item._version };
      });
    testSeriesInstructors = testSeriesInstructors.map((item) => item.instructorId);
    console.log(testSeriesInstructors);

    let allInstructors = await API.graphql({
      query: listInstructors,
      variables: { filter: { _deleted: { ne: true } } },
    });
    console.log(allInstructors.data?.listInstructors?.items);

    let nonMTInstructors = [];
    allInstructors.data?.listInstructors?.items.forEach((item) => {
      console.log(item.id);
      console.log(testSeriesInstructors.findIndex((t) => t == item.id));
      if (testSeriesInstructors.findIndex((t) => t == item.id) > -1) {
      } else {
        nonMTInstructors.push(item);
      }
    });

    console.log(MTInstructors, nonMTInstructors);
    items.value = MTInstructors;
    addExistingsList.value = nonMTInstructors;
  } catch (error) {
...
const deleteItem = async (popup, id, version) => {
  ...
  if (index !== -1) {
    try {
      ...
      await API.graphql({
        query: deleteTestSeriesInstructor,
        variables: {
          input: {
...
<template>
  ...
  <CardBoxModal v-model="isModalExistingActive" :showFooter="false" title="">
    ...
    <p v-if="error" class="text-red-600">
      {{ `error creating new instructor : ${error}` }}
    </p>
    <CardBox is-form @submit.prevent="addExistingInstructor">
          ...
          <tbody>
            <tr v-for="learners in filteredItems" :key="learners.id">
              ...
              <td class="before:hidden whitespace-nowrap flex gap-2">
                <NuxtLink :to="'/examportal/usermgmt/InstructorMgmt/edit/' + learners.id">
