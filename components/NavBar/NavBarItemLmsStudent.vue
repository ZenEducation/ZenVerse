...
const componentClass = computed(() => {
  const base = [
    isDropdownActive.value
      ...
      : `${styleStore.navBarItemLabelStyle} text-white hover:text-slate-200 ${styleStore.navBarItemLabelHoverStyle}`,
...
<template>
  ...
  >
    <div
      ...
      :class="{
        'bg-slate-800  lg:bg-transparent p-3 lg:p-0':
          item.menu,
      }"
    >
    ...
    <div
      v-if="item.menu"
      class="text-sm border-b lg:border lg:absolute lg:top-full lg:left-0 lg:min-w-full lg:z-20 lg:rounded-lg lg:shadow-lg lg:bg-slate-800 border-slate-700"
      :class="{ hidden: !isDropdownActive }"
    >
      <NavBarMenuListLmsStudent  :menu="item.menu" @menu-click="menuClickDropdown" />
    </div>
  </component>
</template>
