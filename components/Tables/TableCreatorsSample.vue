...
<template>
  <CardBoxModal
    ...
    buttonLabel="Okay"
  >
    <p>You can edit Creator details here.(WIP)</p>
  </CardBoxModal>
  ...
  <CardBoxModal
    ...
    title="Are you sure you want to Change status of this Creator?"
    button="danger"
    buttonLabel="Yes"
    has-cancel
  ...
  <div class="text-gray-500 dark:text-white">
    <span>{{ filteredItems.length }} Creators</span>
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="Creators in filteredItems" :key="Creators.id">
        ...
        <td data-label="Last Login" class="lg:w-1 whitespace-nowrap">
          <small
            ...
            :title="Creators.lastLogin"
            >{{ Creators.lastLogin }}</small
          >
        </td>
        <td data-label="Joined" class="lg:w-1 whitespace-nowrap">
          <small
            class="text-gray-500 dark:text-slate-400"
            :title="Creators.joinedOn"
            >{{ Creators.joinedOn }}</small
          >
        </td>
        <TableLearnerEnabled
          data-label="Enabled"
          :checked="Creators.isEnabled"
          @click="EnableItem(true, Creators.id)"
        />
        <td class="before:hidden whitespace-nowrap flex gap-2">
          <NuxtLink to="/cms/usermgmt/CreatorMgmtEdit">
          ...
          <BaseButton
            ...
            :icon="mdiTrashCan"
            small
            @click="deleteItem(true, Creators.id)"
          />
        </td>
      </tr>
    </tbody>
