...
<template>
  <CardBoxModal
    ...
    buttonLabel="Okay"
  >
    <p>You can edit Editor details here.(WIP)</p>
  </CardBoxModal>
  ...
  <CardBoxModal
    ...
    title="Are you sure you want to Change status of this Editor?"
    button="danger"
    buttonLabel="Yes"
    has-cancel
  ...
  <div class="text-gray-500 dark:text-white">
    <span>{{ filteredItems.length }} Editors</span>
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="Editors in filteredItems" :key="Editors.id">
        <td class="border-b-0 lg:w-6 before:hidden">
          <UserAvatar
            :username="Editors.name"
        ...
        <td data-label="Last Login" class="lg:w-1 whitespace-nowrap">
          <small
            ...
            :title="Editors.lastLogin"
            >{{ Editors.lastLogin }}</small
          >
        </td>
        <td data-label="Joined" class="lg:w-1 whitespace-nowrap">
          <small
            class="text-gray-500 dark:text-slate-400"
            :title="Editors.joinedOn"
            >{{ Editors.joinedOn }}</small
          >
        </td>
        <TableLearnerEnabled
          data-label="Enabled"
          :checked="Editors.isEnabled"
          @click="EnableItem(true, Editors.id)"
        />
        <td class="before:hidden whitespace-nowrap flex gap-2">
          <NuxtLink to="/cms/usermgmt/EditorMgmtEdit">
          ...
          <BaseButton
            ...
            :icon="mdiTrashCan"
            small
            @click="deleteItem(true, Editors.id)"
          />
        </td>
      </tr>
    </tbody>
