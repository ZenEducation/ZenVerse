      ...
      <SectionMain>
        ...
        >
          ...
          <PremFormField label="Content : Preview" horizontal>
            <div
              class="flex flex-col border border-black rounded-md p-4 justify-center items-center"
            >
              <div v-if="uploadedFile">
                ...
                />
                <video
                  v-else-if="uploadedFile.type.startsWith('video/')"
                  width="500"
                  :src="uploadedFile.url"
                  controls
                ></video>
          ...
          <PremFormField label="Edit Content :" horizontal>
            ...
            >
              ...
              <Editor
                v-model="modelValue"
                @input="previewText = $event"
                :getText="getPreviewText"
              />
...
watch(messageText, (newVal) => {
  if (newVal) {
    ...
    const text = parser.parseFromString(newVal, "text/html").documentElement
      .textContent;
...
const handleFileChange = (event) => {
  ...
  if (file) {
    ...
    if (allowedExtensions.includes(fileExtension)) {
      ...
      alert(
        "Invalid file format. Please select an image, video, gif, or tiff file."
      );
