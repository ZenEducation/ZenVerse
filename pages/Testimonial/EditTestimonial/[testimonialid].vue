      ...
      <SectionMain>
        ...
        >
          ...
          >
            ...
            <button
              class="bg-red-700 text-white w-24 h-10 rounded"
              @click="discardBtn"
            >
              Discard
            </button>
            <button
              class="bg-blue-600 text-white w-24 h-10 rounded"
              @click="publishBtn"
            >
              Publish
            </button>
        ...
        >
            ...
            <PremFormField horizontal>
              <div class="flex flex-col">
                <div class="flex">
                  <input
                    type="text"
                    placeholder="Click to Add URLs"
                    class="w-full dark:bg-slate-800 rounded-[0.25rem] border border-slate-700"
                    v-model="urlToAdd"
                  />
                  <button
                    v-on:click="addUrl"
                    class="w-24 ml-5 p-1 dark:bg-slate-800 dark:border dark:border-slate-700"
                  >
                    Add URL
                  </button>
                </div>
                <h4>
                  https://youtu.be/<b class="text-red-700">8GmVNb_6Ckk</b>
                  Please add only bold part of link
                </h4>
                <div
                ...
                >
                  <span>{{ url }}</span>
                  <button @click="removeUrl(index)" class="ml-2 text-red-500">
                    Remove
                  </button>
                </div>
              </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-5">
            <div class="flex gap-3 items-center w-full">
              ...
              <select
                ...
                required
                v-model="value"
                class="dark:bg-slate-800 rounded-[0.25rem] border border-slate-700"
              >
                <option
                  v-for="option in categorySelect"
                  :value="option.value"
                  :key="option.value"
                >
                  {{ option.name }}
              ...
              <button
                v-on:click="handleCreateCategory"
          ...
          <div class="flex" v-if="showCreateCategory">
            <input
              ...
              class="p-2 w-4/5 dark:bg-slate-800 rounded-[0.25rem] border border-slate-700"
              type="text"
            />
            <button
              @click="addNewType"
              class="bg-blue-500 text-white px-4 py-2 ml-2 w-1/6"
            >
              Add Category
            </button>
          </div>
        ...
        <div
          class="order-2 lg:hidden flex flex-col lg:flex-row items-center w-full lg:w-44 gap-3"
        >
          <button
            class="bg-green-700 text-white w-full h-10 rounded"
            @click="approveBtn"
          >
            Approve
          </button>
          <button
            class="bg-blue-600 text-white w-full h-10 rounded"
            @click="publishBtn"
          >
            Publish
          </button>
      ...
      try {
        ...
        await DataStore.save(
          new TestimonialData({
            ...
            profilePicKey:
              fileKey.value ?? SingleTestimonial.value.profilePicKey,
        ...
        if (SingleTestimonial.value.student_name == formData.titleText) {
...
onMounted(async () => {
  try {
    ...
    SingleTestimonialImg.value = await Storage.get(
      SingleTestimonial.value.profilePicKey
    );
