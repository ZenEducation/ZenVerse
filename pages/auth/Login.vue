      ...
      <SectionFullScreen v-slot="{ cardClass }" bg="purplePink">
        <CardBox :class="cardClass">
          ...
          <FormField label="Login" help="Please enter your login">
            <FormControl v-model="form.loginEmail" :icon="mdiAccount" placeholder="user@example.com" name="login"
          ...
          <FormField label="Password" :route="{ to: '/auth/forgetpassword', title: 'Forget Password' }">
            <FormControl v-model="form.password" :iconPasswordEye="passwordFieldIcon" :type="passwordFieldType"
              name="password" autocomplete="current-password" @togglePasswordVisibility="togglePasswordField()" />
          </FormField>
          ...
          <template #footer>
            <div class="flex justify-between">
              <BaseButtons>
                ...
                <BaseButton v-if="AuthStore.user" to="/dashboard" color="info" outline label="Dashboard" />
              </BaseButtons>
              <NuxtLink to="/auth/register" class="text-sm bg-gray-800 text-white p-3 rounded-md hover:bg-gray-600">
