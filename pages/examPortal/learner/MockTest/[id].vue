  ...
  <NuxtLayout name="lmsstudent">
    <div class="flex max-md:block">
      <div class="w-2/3 max-md:w-full max-md:h-auto h-screen p-10 pb-30">
        ...
        <div class="grid grid-cols-4 max-md:grid-cols-3 max-sm:grid-cols-2 gap-4 my-3">
        ...
        <template v-if="examData.attempts.length > 0">
          ...
          <div class="flex flex-wrap justify-between px-6 py-2 my-4 items-center"
            ...
            <div>
              <p class="font-semibold">{{ `Attempt # ` + (index + 1) }}</p>
              <p class="font-light text-sm">{{ attempt.updatedAt.slice(0, 10) }}</p>
            </div>
            <NuxtLink :to="'/examportal/learner/examResult/' + attempt.id">
      ...
      <div class="w-1/3 max-md:w-full max-md:h-auto h-screen pt-20 text-xs px-4">
        <CardBox class="max-w-sm">
          ...
          <p class="border-b-2 pb-2 pt-2 flex items-center">
            ...
            <span v-if="examData.isValidityDays">
              {{ `Valid for ${examData.validityDays} Days` }}</span>
          ...
          <p class="pb-2 flex items-center">
            <BaseIcon :path="mdiCheckCircleOutline" /><span class="font-medium pr-1">
              {{ examData.marks }}</span>
            Marks
          </p>
          <p class="pb-2 flex items-center">
            <BaseIcon :path="mdiChatQuestion" /><span class="font-medium pr-1">
             {{ examData.questions }}</span>
            Questions
          </p>
        </CardBox>
      </div>
...
import { createAttempt } from "@/src/graphql/mutations";
...
`;
    ...
    let response = await API.graphql(graphqlOperation(myQuery, { id: examId }));
...
const takeNewExam = async () => {
  try {
    ...
    console.log(newAttempt.data.createAttempt);
    window.location.href =
      "/examportal/learner/examInstruction/" + newAttempt.data.createAttempt.id;
  } catch (error) {
    console.error(error)
  }
};
