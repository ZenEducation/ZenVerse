...
const filteredItems = computed(() => {
  ...
  if (searchQuery.value) {
    filtered = filtered.filter((item) => {
      return search
        ? item.ProductID.match(search) || item.ProductName.match(search)
        : true;
    });
  }
  ...
  <NuxtLayout name="lmsadmin">
    ...
    >
      <div class="heading">
        ...
        <h3 class="font-thin text-xs text-gray-500 py-1 dark:text-white"></h3>
      </div>
      <BaseButtons type="ml-auto xl:mr-4 mr-1">
        <BaseButton class="flex-1" label="Export" color="info" />
      </BaseButtons>
