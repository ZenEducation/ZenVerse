...
onMounted(async () => {
  ...
  let temp = await userMgmtStore.FetchSingleAffiliate(postId);
  console.log(temp);
  if(temp[0]){
    Affiliate.value.profile = temp[1]; // You need to implement this function
    Affiliate.value.AdditionalDetails.LeadStatus = temp[1]?.leadStatus;
    Affiliate.value.AdditionalDetails.UserSegment = temp[1]?.userSegment;
  }
  console.log( postId , Affiliate.value );
});
    ...
    >
      ...
      >
        <div class="pt-8">
          ...
          <div class="w-full flex justify-center h-full">
            <h4 class="font-semibold text-xl">Affiliate 1</h4>
          </div>
          <div class="w-full flex justify-center h-full">
            <h4 class="font-semibold text-sm">Affiliate@ps.com</h4>
          </div>
