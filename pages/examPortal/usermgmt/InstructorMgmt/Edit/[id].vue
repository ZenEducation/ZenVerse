...
onMounted(async () => {
  ...
  let temp = await userMgmtStore.FetchSingleInstructor(postId);
  console.log(temp);
  if(temp[0]){
    Instructor.value.profile = temp[1]; // You need to implement this function
    Instructor.value.AdditionalDetails.LeadStatus = temp[1]?.leadStatus;
    Instructor.value.AdditionalDetails.UserSegment = temp[1]?.userSegment;
  }
  console.log( postId , Instructor.value );
});
...
const disableHandler = async () => {
  try {
    await userMgmtStore.UpdateInstructorStatus(
      Instructor.value.profile.id,
      Instructor.value.profile._version,
      !Instructor.value.profile.isEnabled,
    );
    Instructor.value.profile.isEnabled = !Instructor.value.profile.isEnabled ;
...
const deleteHandler = async () => {
  try {
    await userMgmtStore.DeleteInstructor(
      Instructor.value.profile.id,
      Instructor.value.profile._version,
    );
    
    isModalDeactivateActive.value = false;
    window.location.href = "/examportal/usermgmt/InstructorMgmt"
  } catch (error) {
    console.error(error);
  }
};
      ...
      >
        ...
        >
          ...
          <div v-if="isActive == 0" class="flex items-center gap-2">
            ...
            >
            {{Instructor.profile.isEnabled ? "Disable" : "Enable"}}
            </p>
            <p
