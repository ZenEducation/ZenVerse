...
const learner = ref({
  profile: {
    name:  LearnerData.value?.name || "Learner",
...
onMounted(async () => {
  ...
  if(temp[0]){
    ...
    learner.value.AdditionalDetails.LeadStatus = temp[1]?.leadStatus;
    learner.value.AdditionalDetails.UserSegment = temp[1]?.userSegment;
  }
  console.log( postId , learner.value );
});
    ...
    >
      ...
      >
        <div class="pt-8">
          ...
          <div class="w-full flex justify-center h-full">
            <h4 class="font-semibold text-xl">{{learner.profile.name}}</h4>
          </div>
          <div class="w-full flex justify-center h-full">
            <h4 class="font-semibold text-sm">{{learner.profile.email}}</h4>
          </div>
